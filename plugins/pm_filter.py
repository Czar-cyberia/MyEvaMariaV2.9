...
async def req_movies(bot,query):
    
    # chat_id = query.message.from_user.id
    
    cmd,chat_id ,search = query.data.split('_')
    # print("inside the query cb handler")
    ...
    try:
        ...
        fname = query.from_user.first_name
        uname = "unavailable"
    await bot.send_message(LOG_CHANNEL,text=f"**NEW MOVIE REQUEST** \n\n **User :** <a href=tg://user?id={chat_id}>{fname}</a> \n\n **USERNAME : ** {uname}\n\n **CHAT ID : ** `{chat_id}` \n\n ** MOVIE: ** `{search}`",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Uploading Done👍",callback_data=f'sendsuccess_{chat_id}_{search}')]]))

    await query.message.edit_text(
        f"😁 Thanks For Requesting Movie. \n \n Our Admin have got your message❤.\n\n Once your movie will be availaible we will notify you.\n\n **This usually takes few hours to upload movie. **",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✔ Done",callback_data='close_data')]]))
@Client.on_callback_query(filters.regex(r"^sendsuccess"))
async def sendsuccess(bot,query):
    
    # chat_id = query.message.from_user.id
    # fname = query.message.from_user.first_name
    fname = "CODE NOT UPDATED"
    cmd,chat_id,search = query.data.split('_')
    print("inside the success cb handler")
    # await query.message.delete()
    await bot.send_message(chat_id,text=f"**🎉Congratulation ** \n\n **Your requested movie has been uploaded by our admins**  \n\n  ** MOVIE: ** `{search}`\n\n Now you can download your movie by this bot")
    await query.message.delete()
    # await query.message.edit_text(
    #     f"😁 Thanks For Requesting Movie. \n \n Our Admin have got your message❤.\n\n Once your movie will be availaible we will notify you.\n\n **This usually takes few hours to upload movie. **",
    #     reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✔ Done",callback_data='close_data')]]))
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            babuji_reply = await message.reply_text("Searching For Movie ...🔍")
            search = message.text
